// No external imports needed for this class
...
/**
 * Processes note content for LLM consumption
 * Handles cleaning, chunking, and metadata extraction
 */
export class ContentProcessor {
  /** Default maximum tokens for LLM context window */
  private readonly DEFAULT_MAX_TOKENS = 4000;

  /** Default chunk size for large content */
   ...
   * 
   * @param content - Raw note content to process
   * @param title - Note title (optional)
   * @param options - Processing options for customization
  ...
  /**
   * Extract metadata from content including word count, images, links, and code blocks
   * 
   * @param content - Raw note content to analyze
   * @param title - Note title to include in metadata
   * @returns ContentMetadata object with extracted information
   */
